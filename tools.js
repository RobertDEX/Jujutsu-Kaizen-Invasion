...
async function seedIfEmpty() {
  ...
  DEFAULT_TOOLS.forEach(t => {
    batch.set(db.collection(COLL).doc(), {
      ...
      imageURL:null, reservedBy:null, reserveExpiry:null, claimedAt:null,
...
function formatTimer(exp) {
  const d=exp-Date.now(); if(d<=0) return 'Expired';
...
function getFiltered(view) {
  ...
  if (STATE.searchQuery) {
    ...
    items=items.filter(t=>t.name.toLowerCase().includes(q)||(t.owner&&t.owner.toLowerCase().includes(q))||t.grade.toLowerCase().includes(q));
  }
...
document.getElementById('btn-admin-login-confirm').addEventListener('click',()=>{
  ...
  if(pw===ADMIN_PASSWORD){err.style.display='none';document.getElementById('admin-password-input').value='';closeModal('modal-admin-login');setAdmin(true);showNotification('Admin mode activated.','success');}
...
function renderGrid() {
  ...
  items.forEach((t,i)=>{
    ...
    const imgHTML = t.imageURL
      ? `<img class="tool-card-image" src="${t.imageURL}" alt="${t.name}" loading="lazy"/>`
      : `<div class="tool-card-no-image">器</div>`;
...
function renderAdminPanel() {
  ...
  [...STATE.tools].sort((a,b)=>a.name.localeCompare(b.name)).forEach((t,i)=>{
    ...
    const imgHTML=t.imageURL?`<img class="tool-card-image" src="${t.imageURL}" alt="${t.name}" loading="lazy"/>`:'';
...
setInterval(()=>{
  ...
  document.querySelectorAll('.card-timer[data-expiry]').forEach(el=>{
    ...
    if(exp<=now){
      ...
      if(card){const t=STATE.tools.find(x=>x.id===card.dataset.id);if(t&&t.status==='reserved')db.collection(COLL).doc(t.id).update({status:'unclaimed',reservedBy:null,reserveExpiry:null});}
  ...
  if(document.getElementById('modal-detail').classList.contains('open')&&STATE.activeTechId){
    ...
    if(t&&t.status==='reserved'){
      ...
      if(el){if(t.reserveExpiry<=now){db.collection(COLL).doc(t.id).update({status:'unclaimed',reservedBy:null,reserveExpiry:null});closeModal('modal-detail');}else{el.textContent=formatTimer(t.reserveExpiry);}}
    }
  }
},1000);
...
async function uploadImage(file, toolId) {
  const ext  = file.name.split('.').pop();
  const ref  = storage.ref('tools/'+toolId+'.'+ext);
  const progressBar = document.getElementById('add-upload-fill');
  const progressWrap= document.getElementById('add-upload-progress');
...
function openDetail(id) {
  ...
  if(t.imageURL){imgEl.src=t.imageURL;imgEl.style.display='block';}
  ...
  if(t.docLink){docA.href=t.docLink;docDiv.style.display='block';}else{docDiv.style.display='none';}
  ...
  if(t.status==='unclaimed'){
    ...
    if(IS_ADMIN){
      ...
      p.textContent='Awaiting admin confirmation.'; actions.appendChild(p);
    }
...
function makeBtn(cls,label,fn){const b=document.createElement('button');b.className=cls;b.textContent=label;b.addEventListener('click',fn);return b;}
...
function openClaimModal(id,mode){
  ...
  document.getElementById('claim-modal-desc').textContent=(res?'Reserving':'Claiming')+': "'+t.name+'"'+(res?' — 2 weeks.':'');
...
document.getElementById('btn-claim-confirm').addEventListener('click',async()=>{
  ...
  if(mode==='claim'){
    ...
    showNotification('"'+t.name+'" reserved by '+name+'.','info');
  }
...
document.getElementById('btn-delete-confirm').addEventListener('click',async()=>{
  ...
  if(t.imageURL){
    try{const ref=storage.refFromURL(t.imageURL);await ref.delete();}catch(e){}
  }
...
document.getElementById('open-add-modal').addEventListener('click',()=>{
  ...
  document.getElementById('add-upload-progress').style.display='none';
  document.getElementById('add-upload-fill').style.width='0%';
  openModal('modal-add');
});
...
document.getElementById('btn-add-confirm').addEventListener('click',async()=>{
  ...
  try {
    ...
    const ref = await db.collection(COLL).add({
      ...
      owner:owner||null,docLink,imageURL:null,
    ...
    if(STATE.pendingImageFile){
      const url=await uploadImage(STATE.pendingImageFile,ref.id);
...
/* ── Search ───────────────────────────────────────────────────── */
document.getElementById('sidebar-search-input').addEventListener('input',e=>{STATE.searchQuery=e.target.value.trim();renderAll();});
...
function spawnRipple(e){
  ...
  r.style.cssText='border-color:rgba(220,40,40,0.4);';
